...
add_action('rest_api_init', function(){
    ...
    // Medical Report Generator endpoint (v6.0.0) — streams PDF from Render backend
    // Requires logged-in user. Fetches full medical snapshot server-side, sends to
    // Render /generate-report, and streams the resulting PDF back to the browser.
    register_rest_route('autoa/v1', 'generate-report', array(
        'methods'             => 'POST',
        'permission_callback' => function() {
            return is_user_logged_in();
        },
        'callback'            => 'autoa_rest_generate_report',
        'args'                => array(
...
// ===========================================================================
// MEDICAL REPORT GENERATOR — v6.0.0
// Endpoint: POST /wp-json/autoa/v1/generate-report
// Requires: logged-in user + identity_token
// Flow:
//   1. Fetch full medical snapshot (same as chat-proxy)
//   2. POST to Render /generate-report with snapshot + identity_token
//   3. Stream the PDF response directly to the browser
// ===========================================================================
function autoa_rest_generate_report( WP_REST_Request $req ) {
    $start_time     = microtime(true);
    $user_id        = get_current_user_id();
    $identity_token = $req->get_param('identity_token');
    ...
    // -----------------------------------------------------------------------
    // STEP 1: Build full medical snapshot (identical to chat-proxy logic)
    // -----------------------------------------------------------------------
    global $wpdb;
    ...
    $health_profile_table = $wpdb->prefix . 'autoanosis_health_profiles';
    if ($wpdb->get_var("SHOW TABLES LIKE '$health_profile_table'") === $health_profile_table) {
        $hp = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM $health_profile_table WHERE user_id = %d ORDER BY created_at DESC LIMIT 1",
            $user_id
        ), ARRAY_A);
        if ($hp) { $snapshot['health_profile'] = $hp; }
    }

    // Daily check-ins (last 14 days for report — more data than chat)
    $checkins_table = $wpdb->prefix . 'autoa_daily_checkins';
    if ($wpdb->get_var("SHOW TABLES LIKE '$checkins_table'") === $checkins_table) {
        ...
        if (!empty($checkins)) { $snapshot['recent_checkins'] = $checkins; }
    }

    // Medications — legacy table
    $medications_table = $wpdb->prefix . 'autoanosis_medications';
    if ($wpdb->get_var("SHOW TABLES LIKE '$medications_table'") === $medications_table) {
        ...
        if (!empty($meds)) { $snapshot['medications'] = $meds; }
    }

    // Medications — mm_medications (PRIMARY, overrides legacy)
    $mm_meds_table = $wpdb->prefix . 'mm_medications';
    if ($wpdb->get_var("SHOW TABLES LIKE '$mm_meds_table'") === $mm_meds_table) {
        $mm_meds = $wpdb->get_results($wpdb->prepare(
            "SELECT medication_name, dosage, frequency, time_slots, active,
                    instructions AS notes, created_at
             FROM $mm_meds_table
             WHERE patient_id = %d
        ...
        if (!empty($mm_meds)) {
            foreach ($mm_meds as &$m) {
                if (isset($m['time_slots']) && is_string($m['time_slots'])) {
                    $dec = json_decode($m['time_slots'], true);
                    if (is_array($dec)) { $m['time_slots'] = $dec; }
                }
            }
            unset($m);
            $snapshot['medications'] = $mm_meds;
        }
    }
    ...
    if ($wpdb->get_var("SHOW TABLES LIKE '$symptoms_table'") === $symptoms_table) {
        $symptoms = $wpdb->get_results($wpdb->prepare(
            "SELECT * FROM $symptoms_table
             WHERE user_id = %d AND recorded_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)
             ORDER BY recorded_at DESC",
            $user_id
        ), ARRAY_A);
        if (!empty($symptoms)) { $snapshot['recent_symptoms'] = $symptoms; }
    }

    // Structured exam results from Exams Normalizer subsystem (Doctor Dashboard source of truth)
    // RULE: Doctor Dashboard reads ONLY structured, normalised exam data.
    // Raw blobs / OCR text / failed extracts are NEVER exposed here.
    $structured_exams = autoanosis_exams_fetch_structured_snapshot( $user_id );
    if ( !is_wp_error($structured_exams) && !empty($structured_exams['structured_exam_results']) ) {
        $snapshot['structured_exam_results'] = $structured_exams['structured_exam_results'];
        $snapshot['exam_report_count']       = $structured_exams['report_count'] ?? 0;
    ...
    if ($wpdb->get_var("SHOW TABLES LIKE '$notes_table'") === $notes_table) {
        ...
        if (!empty($notes)) { $snapshot['health_notes'] = $notes; }
    }

    // BEST Protocol (latest entry)
    $best = get_user_meta($user_id, 'autoanosis_best_protocol_last', true);
    if (empty($best)) { $best = get_user_meta($user_id, 'autoanosis_medical_snapshot_last', true); }
    if (empty($best)) { $best = get_user_meta($user_id, 'autoanosis_medical_snapshot_last_v2', true); }
    if (is_array($best) && isset($best['payload']) && is_array($best['payload'])) { $best = $best['payload']; }
    if (is_string($best)) {
        $tmp = @unserialize($best);
        if ($tmp !== false || $best === 'b:0;') { $best = $tmp; }
    }
    if (is_array($best) && !empty($best)) {
        $snapshot['best_protocol'] = $best;
    }

    // BEST history (same 3-source merge as chat-proxy)
    $best_history_merged = array();
    $meta_history = get_user_meta($user_id, 'autoanosis_medical_snapshot_history', true);
    if (is_array($meta_history)) {
        foreach ($meta_history as $h) {
            if (!is_array($h)) continue;
            $payload = (isset($h['payload']) && is_array($h['payload'])) ? $h['payload'] : $h;
            $ts  = isset($h['timestamp']) ? intval($h['timestamp']) : 0;
            $src = isset($h['source'])    ? sanitize_text_field($h['source']) : 'BEST_PROTOCOL';
    ...
    $legacy_history = get_user_meta($user_id, 'autoanosis_best_history', true);
    if (is_array($legacy_history)) {
        foreach ($legacy_history as $h) {
            if (!is_array($h)) continue;
            $payload = isset($h['payload']) && is_array($h['payload']) ? $h['payload'] : null;
            if (empty($payload)) continue;
            $ts  = isset($h['ts']) ? intval($h['ts']) : (isset($h['timestamp']) ? intval($h['timestamp']) : 0);
            $src = isset($h['source']) ? sanitize_text_field($h['source']) : 'BEST_PROTOCOL';
            $best_history_merged[] = array('source' => $src, 'timestamp' => $ts, 'payload' => $payload);
        }
    }
    if (!empty($best) && is_array($best)) {
        $best_last_meta = get_user_meta($user_id, 'autoanosis_medical_snapshot_last_v2', true);
        if (empty($best_last_meta)) { $best_last_meta = get_user_meta($user_id, 'autoanosis_medical_snapshot_last', true); }
        $best_last_ts  = is_array($best_last_meta) && isset($best_last_meta['timestamp']) ? intval($best_last_meta['timestamp']) : 0;
        $best_last_src = is_array($best_last_meta) && isset($best_last_meta['source'])    ? sanitize_text_field($best_last_meta['source']) : 'BEST_PROTOCOL';
        $best_history_merged[] = array('source' => $best_last_src, 'timestamp' => $best_last_ts, 'payload' => $best);
    }
    // De-dup, sort desc, keep last 10
    $dedup_keys = array();
    $best_uniq  = array();
    foreach ($best_history_merged as $h) {
        if (!is_array($h) || empty($h['payload'])) continue;
        $k = intval($h['timestamp']) . '|' . md5(wp_json_encode($h['payload']));
        if (isset($dedup_keys[$k])) continue;
        $dedup_keys[$k] = true;
        $best_uniq[] = $h;
    }
    ...
    // -----------------------------------------------------------------------
    // STEP 2: POST to Render /generate-report
    // -----------------------------------------------------------------------
    $request_body = array(
    ...
    error_log('=== AUTOANOSIS GENERATE REPORT ===');
    error_log('User ID: ' . $user_id);
    error_log('Snapshot keys: ' . implode(', ', array_keys($snapshot)));

    $response = wp_remote_post(
        'https://autoanosis-ai-backend.onrender.com/generate-report',
        array(
            'headers' => array( 'Content-Type' => 'application/json' ),
            'body'    => wp_json_encode($request_body),
            'timeout' => 120, // PDF generation can take up to 2 minutes (4x AI calls)
            'sslverify' => true,
        )
    );

    // -----------------------------------------------------------------------
    // STEP 3: Handle response — stream PDF or return error
    // -----------------------------------------------------------------------
    if ( is_wp_error($response) ) {
        $err = $response->get_error_message();
        error_log('Generate report WP error: ' . $err);
        return new WP_Error(
            'backend_error',
            'Failed to connect to AI backend: ' . $err,
            array('status' => 503)
        );
    }
    ...
    error_log('Generate report response code: ' . $response_code . ' | time: ' . $elapsed . 's | bytes: ' . strlen($response_body));
    error_log('=== END GENERATE REPORT ===');

    if ( $response_code !== 200 ) {
        // Try to parse error JSON from Render
        $err_data = json_decode($response_body, true);
        $err_msg  = (is_array($err_data) && isset($err_data['error']))
            ? $err_data['error']
            : 'AI backend returned HTTP ' . $response_code;
        return new WP_Error(
            'report_error',
            $err_msg,
            array('status' => $response_code)
    ...
    // Detect content type: PDF or plain text fallback
    $content_type = wp_remote_retrieve_header($response, 'content-type');
    $is_pdf       = (strpos($content_type, 'application/pdf') !== false);

    // Build safe filename
    $patient_name = isset($snapshot['user_name']) ? $snapshot['user_name'] : 'user_' . $user_id;
    $safe_name    = preg_replace('/[^a-zA-Z0-9_\-]/', '_', $patient_name);
    $date_str     = date('Ymd');
    $filename     = 'autoanosis_report_' . $safe_name . '_' . $date_str . ($is_pdf ? '.pdf' : '.txt');

    // Stream directly to browser — bypass WP REST JSON wrapper
    if ( ! headers_sent() ) {
        status_header(200);
        header('Content-Type: '       . ($is_pdf ? 'application/pdf' : 'text/plain; charset=utf-8'));
        header('Content-Disposition: attachment; filename="' . $filename . '"');
        header('Content-Length: '     . strlen($response_body));
        header('Cache-Control: no-store, no-cache, must-revalidate');
        header('Pragma: no-cache');
        header('X-Report-Time: '      . $elapsed . 's');
        echo $response_body;
        exit;
    }

    // Fallback: if headers already sent, return as WP_REST_Response with base64 body
    // (should not happen in normal WP REST flow, but defensive)
    return new WP_REST_Response(array(
        'success'      => true,
        'filename'     => $filename,
        'content_type' => $is_pdf ? 'application/pdf' : 'text/plain',
        'data_base64'  => base64_encode($response_body),
        'elapsed'      => $elapsed,
    ), 200);
}
