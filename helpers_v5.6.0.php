...
function autoa_rest_chat_proxy( WP_REST_Request $req ) {
    ...
    if ( $user_id ) {
        ...
        // Medical Memory plugin stores medications in {prefix}mm_medications (e.g. wp_mm_medications)
        // The old hardcoded 'tkc_mm_medications' was wrong and caused medications to never be found.
        ...
        if ($wpdb->get_var("SHOW TABLES LIKE '$medications_table'") === $medications_table) {
            $medications = $wpdb->get_results($wpdb->prepare(
                "SELECT medication_name, dosage, frequency FROM $medications_table WHERE user_id = %d AND status = 'active' ORDER BY created_at DESC",
                $user_id
            ), ARRAY_A);
        ...
        if ($wpdb->get_var("SHOW TABLES LIKE '$symptoms_table'") === $symptoms_table) {
            $symptoms = $wpdb->get_results($wpdb->prepare(
                "SELECT * FROM $symptoms_table WHERE user_id = %d ORDER BY recorded_at DESC LIMIT 10",
                $user_id
            ), ARRAY_A);
        ...
        // 7. Get BEST Protocol Data
        // autoanosis_medical_snapshot_last is stored as a PHP-serialized array by WordPress.
        // get_user_meta() automatically unserializes it, so $best_protocol_raw is already an array.
        // Structure: { source, timestamp, payload: { visit_date, baseline, events, symptoms, targets, medications, notes, ... } }
        $best_protocol_raw = get_user_meta($user_id, 'autoanosis_medical_snapshot_last', true);
        if (!empty($best_protocol_raw)) {
            // get_user_meta already unserializes PHP-serialized data
            if (is_array($best_protocol_raw)) {
                // Extract the payload if nested, otherwise use the whole array
                $best_payload = isset($best_protocol_raw['payload']) ? $best_protocol_raw['payload'] : $best_protocol_raw;
                if (is_array($best_payload) && !empty($best_payload)) {
                    $snapshot['best_protocol'] = $best_payload;
                }
            } elseif (is_string($best_protocol_raw)) {
                // Fallback: try JSON decode if stored as JSON string
                $best_decoded = json_decode($best_protocol_raw, true);
                if (json_last_error() === JSON_ERROR_NONE && is_array($best_decoded)) {
                    $best_payload = isset($best_decoded['payload']) ? $best_decoded['payload'] : $best_decoded;
                    if (!empty($best_payload)) {
                        $snapshot['best_protocol'] = $best_payload;
                    }
                }
            }
        ...
        if ($wpdb->get_var("SHOW TABLES LIKE '$test_results_table'") === $test_results_table) {
            $test_results = $wpdb->get_results($wpdb->prepare(
                "SELECT * FROM $test_results_table WHERE user_id = %d ORDER BY test_date DESC LIMIT 10",
                $user_id
            ), ARRAY_A);
        ...
        // 10. BEST Protocol text fallback — embed in health_info so LLM always sees it
        // Even if app.py structured parsing fails, the text will be in health_info.
        if (!empty($snapshot['best_protocol']) && is_array($snapshot['best_protocol'])) {
            $bp = $snapshot['best_protocol'];
            $lines = array();
            $lines[] = '--- B.E.S.T. Protocol (Προετοιμασία Ραντεβού) ---';
            if (!empty($bp['visit_date']))    $lines[] = 'Ημερομηνία Ραντεβού: ' . $bp['visit_date'];
            if (!empty($bp['visit_doctor']))  $lines[] = 'Ιατρός/Ειδικότητα: ' . $bp['visit_doctor'];
            if (!empty($bp['visit_goal']))    $lines[] = 'Στόχος: ' . $bp['visit_goal'];
            if (!empty($bp['visit_period']))  $lines[] = 'Περίοδος: ' . $bp['visit_period'];
            if (!empty($bp['b_meds']))        $lines[] = '[B] Φάρμακα: ' . $bp['b_meds'];
            if (!empty($bp['b_side']))        $lines[] = '[B] Παρενέργειες: ' . $bp['b_side'];
            if (!empty($bp['b_labs']))        $lines[] = '[B] Εξετάσεις: ' . $bp['b_labs'];
            if (!empty($bp['b_baseline']))    $lines[] = '[B] Baseline: ' . $bp['b_baseline'];
            if (!empty($bp['e_infections']))  $lines[] = '[E] Λοιμώξεις: ' . $bp['e_infections'];
            if (!empty($bp['e_stress']))      $lines[] = '[E] Στρες: ' . $bp['e_stress'];
            $e_life = !empty($bp['e_lifestyle']) ? $bp['e_lifestyle'] : (!empty($bp['e_events']) ? $bp['e_events'] : '');
            if (!empty($e_life))              $lines[] = '[E] Αλλαγές ζωής: ' . $e_life;
            if (!empty($bp['e_other']))       $lines[] = '[E] Άλλα: ' . $bp['e_other'];
            if (!empty($bp['s_symptoms']))    $lines[] = '[S] Συμπτώματα: ' . $bp['s_symptoms'];
            if (!empty($bp['s_timing']))      $lines[] = '[S] Χρονισμός: ' . $bp['s_timing'];
            if (!empty($bp['s_impact']))      $lines[] = '[S] Αντίκτυπος: ' . $bp['s_impact'];
            if (!empty($bp['t_goals']))       $lines[] = '[T] Στόχοι: ' . $bp['t_goals'];
            if (!empty($bp['t_biomarkers']))  $lines[] = '[T] Βιοδείκτες: ' . $bp['t_biomarkers'];
            if (!empty($bp['t_questions']))   $lines[] = '[T] Ερωτήσεις: ' . $bp['t_questions'];
            $t_plan = !empty($bp['t_plan']) ? $bp['t_plan'] : (!empty($bp['t_treatments']) ? $bp['t_treatments'] : '');
            if (!empty($t_plan))              $lines[] = '[T] Πλάνο: ' . $t_plan;
            $best_text = implode("\n", $lines);
            $snapshot['best_summary'] = $best_text;
            // Append to health_info as fallback so LLM always sees BEST data
            if (!isset($snapshot['health_info']) || !is_string($snapshot['health_info'])) {
                $snapshot['health_info'] = '';
            }
            if (strpos($snapshot['health_info'], 'B.E.S.T. Protocol') === false) {
                $snapshot['health_info'] = trim($snapshot['health_info'] . "\n\n" . $best_text);
            }
        }
