...
INTENT_PATTERNS = {
    # health_trajectory: queries about health progress over time — MUST be checked before summary
    "health_trajectory": [
        r"πορεία.*υγεία", r"πορεια.*υγεια",
        r"πορεία.*ασθεν", r"πορεια.*ασθεν",
        r"πώς πάει.*υγεία", r"πως παει.*υγεια",
        r"πώς πάει.*πορεία", r"πως παει.*πορεια",
        r"πώς πάει η υγεία", r"πως παει η υγεια",
        r"πώς πάω", r"πως παω",
        r"πώς έχω", r"πως εχω",
        r"πώς είμαι σαν ασθεν", r"πως ειμαι σαν ασθεν",
        r"πορεία μου", r"πορεια μου",
        r"υγεία μου.*χρόνο", r"υγεια μου.*χρονο",
        r"πώς εξελίσσεται", r"πως εξελισσεται",
        r"πώς εξελίσσομαι", r"πως εξελισσομαι",
        r"health.*progress", r"health.*trajectory",
        r"πρόοδος.*υγεία", r"προοδος.*υγεια",
        r"βελτιώνομαι", r"βελτιωνομαι",
        r"επιδεινώνομαι", r"επιδεινωνομαι",
        r"πώς πάει η κατάστασή", r"πως παει η κατασταση",
        r"πώς πάει η πορεία", r"πως παει η πορεια",
        r"ανάλυσε.*υγεία", r"αναλυσε.*υγεια",
        r"ανάλυσε.*δεδομένα", r"αναλυσε.*δεδομενα",
    ],
    "summary": [
    ...
    "pattern_detection": [
        ...
        # Comparison / longitudinal queries
        r"συγκρίν", r"συγκριν", r"compar",
        r"ιστορικό εξετ", r"ιστορικο εξετ",
        r"πορεία εξετ", r"πορεια εξετ",
        r"πώς έχουν αλλάξ", r"πως εχουν αλλαξ",
        r"διαχρονικά", r"διαχρονικα",
        r"από τότε", r"απο τοτε",
        r"πορεία τους", r"πορεια τους",
        r"εξελιχθεί", r"εξελιχθει",
    ],
    "education": [
    ...
    # document_analysis: queries about uploaded medical documents / files
    "document_analysis": [
        r"έγγραφ", r"εγγραφ",
        r"αρχείο", r"αρχειο",
        r"ανέβασ", r"ανεβασ",
        r"pdf",
        # 'άρθρο' only as document reference, NOT standalone (avoid matching 'αρθρίτιδα', 'αρθρώσεις')
        r"το άρθρο", r"το αρθρο",
        r"του άρθρου", r"του αρθρου",
        r"την ερευνα", r"την ερευνα",
        r"εξιτήρι", r"εξιτηρι",
        r"γνωμάτευ", r"γνωματευ",
        r"παραπεμπτικ", r"συνταγ",
        r"απεικονιστικ",
        r"συμπέρασμα.*βγαίν", r"συμπερασμα.*βγαιν",
        r"τι λέει.*έγγραφ", r"τι λεει.*εγγραφ",
        r"τι γράφει", r"τι γραφει",
        r"τι αναφέρει", r"τι αναφερει",
        r"ανάλυσε.*έγγραφ", r"αναλυσε.*εγγραφ",
        r"διάβασε.*έγγραφ", r"διαβασε.*εγγραφ",
        r"τι δείχνει.*έγγραφ", r"τι δειχνει.*εγγραφ",
        r"σπλήν", r"σπλην",
        r"σπληνεκτομ",
        r"χειρουργ",
        r"νοσηλεί", r"νοσηλει",
        r"εξέτασ.*έγγραφ", r"εξετασ.*εγγραφ",
        r"από.*έγγραφ", r"απο.*εγγραφ",
        r"βάσει.*έγγραφ", r"βασει.*εγγραφ",
        r"την δημοσίευση", r"την δημοσιευση",
        r"μελέτη", r"μελετη",
        r"δημοσίευση", r"δημοσιευση",
    ],
}
...
def detect_intent(message: str) -> str:
    ...
    # Priority 2: health_trajectory (longitudinal health progress queries)
    # Must be checked BEFORE summary, as trajectory queries often contain 'υγεία'
    for p in INTENT_PATTERNS["health_trajectory"]:
        if re.search(_normalize_greek(p), msg_norm):
            return "health_trajectory"
            
    # Priority 3: pattern_detection
    for p in INTENT_PATTERNS["pattern_detection"]:
        if re.search(_normalize_greek(p), msg_norm):
            return "pattern_detection"
            
    # Priority 4: document_analysis (BEFORE education — document queries are specific)
    for p in INTENT_PATTERNS["document_analysis"]:
        if re.search(_normalize_greek(p), msg_norm):
...
def build_system_prompt(snapshot: str, intent: str) -> str:
    ...
    templates = {
        "health_trajectory": """
ΑΥΤΟ ΕΙΝΑΙ HEALTH TRAJECTORY MODE — Ανάλυση πορείας υγείας.
Ο χρήστης ζητά να καταλάβει πώς εξελίσσεται η υγεία του συνολικά.
ΑΠΑΓΟΡΕΥΕΤΑΙ να πετάς απλά ευρήματα χωρίς συνολική αξιολόγηση.
ΑΠΑΓΟΡΕΥΕΤΑΙ να πεις "δεν έχω πρόσβαση" εφόσον υπάρχει context.
ΑΠΑΓΟΡΕΥΕΤΑΙ να παραλείπεις ενότητες που έχουν δεδομένα — αν λείπει δεδομένο, γράψε "Δεν υπάρχουν διαθέσιμα δεδομένα".

ΑΥΣΤΗΡΗ ΔΟΜΗ ΑΠΑΝΤΗΣΗΣ (max 350 λέξεις):

📊 ΒΑΣΗ ΔΕΔΟΜΕΝΩΝ ΠΟΥ ΑΝΑΛΥΘΗΚΕ:
[Αναφέρε: περίοδος (από — έως), αριθμός check-ins, αριθμός εξετάσεων (reports), φάρμακα σε χρήση, BEST καταχωρήσεις. Αν λείπει κάτι: "Δεν υπάρχουν διαθέσιμα δεδομένα για [κατηγορία]."]

📈 ΓΕΝΙΚΗ ΤΑΣΗ:
[Μία σαφής κρίση: βελτίωση / σταθερότητα / επιδείνωση / ανεπαρκή δεδομένα. Βάσισε αυτό ΑΠΟΚΛΕΙΣΤΙΚΑ στα δεδομένα — μην εικάζεις. Αν υπάρχει trend_summary: χρησιμοποίησέ το. Αν δεν υπάρχει αρκετό ιστορικό: γράψε "Δεν υπάρχουν αρκετά δεδομένα για αξιόπιστη εκτίμηση τάσης."]

🩺 ΣΥΜΠΤΩΜΑΤΑ ΑΠΟ CHECK-INS:
[Αν υπάρχουν check-ins: παρουσίασε τους μέσους όρους της εβδομάδας για πόνο, κόπωση, ενέργεια, διάθεση (κλίμακα 1-10). Σημείωσε αν κάποια τιμή είναι ανησυχητική (>6 για πόνο/κόπωση, <4 για ενέργεια/διάθεση). Αν δεν υπάρχουν check-ins: "Δεν υπάρχουν καταγεγραμμένα check-ins."]

🔬 ΕΞΕΤΑΣΕΙΣ:
[Για κάθε εξέταση που υπάρχει στο context: ημερομηνία, τύπος, αριθμός εκτός ορίων. Επισήμανε μόνο τα abnormal ευρήματα με τιμή, μονάδα και reference range. Μην κάνεις dump όλων. Αν δεν υπάρχουν εξετάσεις: "Δεν υπάρχουν καταγεγραμμένες εξετάσεις."]

💊 ΦΑΡΜΑΚΑ / ADHERENCE:
[Λίστα ενεργών φαρμάκων. Αν υπάρχει medication_schedule: αναφέρε ληφθέντα, χαμένα, εκκρεμή δόσεις για σήμερα — ΧΩΡΙΣ ιατρικές οδηγίες αλλαγής. Αν δεν υπάρχει schedule: "Δεν υπάρχουν δεδομένα συμμόρφωσης."]

📋 BEST / ΠΡΟΕΤΟΙΜΑΣΙΑ ΕΠΙΣΚΕΨΗΣ:
[Αν υπάρχει BEST: ημερομηνία τελευταίας καταχώρησης, γιατρός, κύρια θέματα (b_labs, s_symptoms, t_questions). Τι αξίζει να πάει στον γιατρό βάσει των δεδομένων. Αν δεν υπάρχει BEST: "Δεν υπάρχει καταγεγραμμένο BEST."]

⚠️ ΑΣΦΑΛΕΣ ΣΥΜΠΕΡΑΣΜΑ:
[Μία παράγραφος. ΔΕΝ κάνεις διάγνωση. ΔΕΝ αλλάζεις αγωγή. ΔΕΝ λες "έχεις έξαρση". Γράψε τι δείχνουν τα δεδομένα, τι χρειάζεται παρακολούθηση, και παράπεμψε στον γιατρό για κλινική αξιολόγηση.]
""",
        "summary": """
ΑΠΑΓΟΡΕΥΕΤΑΙ να πεις "πήγαινε στις Εξετάσεις" ή να δίνεις οδηγίες πλοήγησης αντί για ανάλυση — τα δεδομένα είναι ήδη στο context σου.
ΑΠΑΓΟΡΕΥΕΤΑΙ να πεις "δεν έχω πρόσβαση" εφόσον υπάρχει context.
...
""",
        "pattern_detection": """
ΑΠΑΓΟΡΕΥΕΤΑΙ να πεις "δεν έχω πρόσβαση" ή να παραπέμψεις τον χρήστη στην εφαρμογή — τα δεδομένα είναι ήδη στο context σου.
...
""",
        "document_analysis": """
ΑΥΤΟ ΕΙΝΑΙ DOCUMENT ANALYSIS MODE — Ανάλυση ανεβασμένου ιατρικού εγγράφου.

ΑΠΑΓΟΡΕΥΕΤΑΙ να πεις "δεν έχω πρόσβαση στο έγγραφο" — το περιεχόμενο είναι ήδη στο context σου (ενότητα ΑΡΧΕΙΟ ΕΓΓΡΑΦΩΝ).
ΑΠΑΓΟΡΕΥΕΤΑΙ να δίνεις γενικόλογες απαντήσεις αποφεύγοντας το περιεχόμενο.
...
🔍 ΤΙ ΣΗΜΑΙΝΕΙ ΑΥΤΟ:
[Σύντομη ερμηνεία σε απλή γλώσσα — τι συνέβη ή τι αναφέρει το έγγραφο, χωρίς ιατρική διάγνωση. Π.χ. αν αναφέρεται σπληνεκτομή, εξήγησε τι είναι και ποιες είναι οι γνωστές συνέπειες.]

⚠️ ΤΙ ΧΡΕΙΑΖΕΤΑΙ ΠΡΟΣΟΧΗ / ΠΑΡΑΚΟΛΟΥΘΗΣΗ:
[max 2 bullets — αν το έγγραφο αναφέρει κάτι που χρειάζεται follow-up, φαρμακευτική αγωγή, ή επανεξέταση]

📌 ΠΕΡΙΟΡΙΣΜΟΣ:
["Η παραπάνω ανάλυση βασίζεται αποκλειστικά στο κείμενο του εγγράφου. Για κλινική αξιολόγηση συμβουλέψου τον γιατρό σου."]
"""
    }
...
- ΑΠΑΓΟΡΕΥΕΤΑΙ να δίνεις οδηγίες πλοήγησης (π.χ. "πήγαινε στις Εξετάσεις", "επίλεξε την κατηγορία") αντί για ανάλυση όταν υπάρχουν δεδομένα στο context. ΑΝΑΛΥΕ τα δεδομένα απευθείας.
...
RELAPSE / FLARE SAFETY RULE (ΔΕΝ εφαρμόζεται σε DOCUMENT_ANALYSIS mode):
Για queries τύπου "έχω έξαρση;", "φαίνεται έξαρση;", "είναι relapse;", "επιδεινώνομαι;" όταν δεν είμαστε σε DOCUMENT_ANALYSIS mode:
